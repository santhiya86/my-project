...
function Counselling() {
  ...
  const fetchCounsellingList = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:9002/api/counselling?year=${selectedYear}&semester=${selectedSemester}&ia=${selectedIA}`
      );
  ...
  const handleScheduleCounselling = async (student) => {
    try {
      const response = await fetch('http://localhost:9002/api/counselling', {
        method: 'POST',
        headers: {
